...
public class ClientScreen extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        check_messages_btn = new javax.swing.JButton();
        ...
        check_messages_btn.setBackground(new java.awt.Color(0, 153, 153));
        check_messages_btn.setForeground(new java.awt.Color(255, 255, 255));
        check_messages_btn.setText("Check for messages");
        check_messages_btn.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                check_messages_btnActionPerformed(evt);
            }
        });
        ...
        jPanel5Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel5Layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(check_messages_btn)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel5Layout.createSequentialGroup()
                ...
                .addGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    ...
                    .addGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                        ...
                        .addComponent(check_messages_btn, javax.swing.GroupLayout.PREFERRED_SIZE, 31, javax.swing.GroupLayout.PREFERRED_SIZE)))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    // Method to call when user clicks on "check messages button"
    private void check_messages_btnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_check_messages_btnActionPerformed
        // TODO add your handling code here:
        methodToCallAfterCheckMessagesClick();
    }//GEN-LAST:event_check_messages_btnActionPerformed
    ...
    private void appendNewUserToCSV() {
        ...
        // append data in csv
        Utils.appendDataToNextLineCSV(uname, timestamp, Constants.STATUS.ONLINE, Constants.DEFAULT_MESSAGE_QUEUE);
    }
    ...
    private void editCSVBeforeClose() {
        ...
        for (int i = 0; i < CSVReaderInJava.users.size(); i++) {
            if (CSVReaderInJava.users.get(i).getUsername().equalsIgnoreCase(uname)) {    // if new username matches with existing username in the csv file then
                ...
                // Call method to rewrite CSV file
                rewriteCSV();
    ...
    private void rewriteCSV() {
        ...
        for (int j = 0; j < CSVReaderInJava.users.size(); j++) {
            users.add(new String[]{CSVReaderInJava.users.get(j).getUsername(), CSVReaderInJava.users.get(j).getTimestamp(), CSVReaderInJava.users.get(j).getStatus(), CSVReaderInJava.users.get(j).getMessageQueue()});
        }
        ...
        public void run() {
            ...
            try {
                while (true) {
                    ...
                    } else if (line.equals(Constants.DEFAULT_MESSAGE_QUEUE)) {
                        JOptionPane.showMessageDialog(null, "No new messages found", "Check Message", JOptionPane.INFORMATION_MESSAGE);
                    } else {
    ...
    // Method to call for retreiving messages from server (after clicking on CheckMessages)
    private void methodToCallAfterCheckMessagesClick() {
        pw.println(Constants.MESSAGE_TO_CHECK_MESSAGES);
    }
        ...
        if (!Utils.isAlpha(UNICAST_USERNAME)) {
            ...
            if (UNICAST_USERNAME.equalsIgnoreCase(uname)) {
                ...
                if (unicast_user_found) {
                    // If user is present then create and message using message format
                    ...
                    // Clear text field of messages
                    message_tf.setText("");

                    // Show dialog if message is sent successfully
    ...
    private void checkConditionsForMulticast(String input_username, String messageToSend) {
        ...
        // Create lists for valid and invalid users
        ArrayList<String> validUsers = new ArrayList<String>();
        ArrayList<String> inValidUsers = new ArrayList<String>();

        // Initialize validUsersString as empty String
        String validUsersString = "";

        // Create a boolean to check if the sender's name is included in the recepients list
        boolean isSelfNameIncluded = false;

        // Split the usernames using comma (,)
        String[] parts = input_username.split(",");

        for (int i = 0; i < parts.length; i++) {
            parts[i] = parts[i].trim();     // Trim all usernames in String array to avoid whitespaces
        }
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            for (int j = 0; j < CSVReaderInJava.users.size(); j++) {
                ...
                // If username in String array matches with the username present on server
                if (parts[i].equalsIgnoreCase(CSVReaderInJava.users.get(j).getUsername())) {    
                    
                    // Add the name to validUsers List
                    validUsers.add(parts[i]);
                }
            }
        }

        // Check for all Valid Usernames
        for (int i = 0; i < validUsers.size(); i++) {

            String valid_username = validUsers.get(i);

            // If valid username contains senders name
            if (valid_username.equalsIgnoreCase(uname)) {
                isSelfNameIncluded = true;
                
                // Show error dialog to remove sender's name from recepients list
                JOptionPane.showMessageDialog(null, "You cannot send message to yourself\n\nKindly change it and try again", "Error", JOptionPane.ERROR_MESSAGE);
                break;
            } else {
                // else append the valid user String
        ...
        if (!isSelfNameIncluded) {
            if (validUsers.size() == parts.length) {
                // If all recepients are valid users then call method sendMulticastMessage
                sendMulticastMessage(messageToSend);
                
                // Show dialog that message sent successfully
                JOptionPane.showMessageDialog(null, "Message sent to all selected users:", "Message sent!!!", JOptionPane.INFORMATION_MESSAGE);
            } else {
                // Show error dialog showing Valid Users list
    ...
    // Method to upload Multicast Message to server
    ...
    // Method to upload Broadcast Message to server
    ...
    private void refreshListOfUsers() {
        ...
        for (int i = 0; i < CSVReaderInJava.users.size(); i++) {
            ...
            if (!username.equalsIgnoreCase(uname)) {
                // Append all data to respective textfields
    ...
    private javax.swing.JButton check_messages_btn;

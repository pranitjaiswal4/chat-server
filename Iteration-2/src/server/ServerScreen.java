...
public class ServerScreen extends javax.swing.JFrame {
    ...
    public void sendMessageBasedOnMessageType(String user, String message) {
        ...
        if (MESSAGE_TYPE.equals("Unicast")) {
            System.out.println("Pranit : StoreMessageTest : ServerScreen : sendMessageBasedOnMessageType : sending Unicast : ");

            String receiver = UNICAST_USERNAME;
            String sender = user;
            storeMessageQueueToCSV(sender, receiver, message);

        } else if (MESSAGE_TYPE.equals("Multicast")) {

            for (int i = 0; i < MULTICAST_USERNAMES.size(); i++) {

                String receiver = MULTICAST_USERNAMES.get(i);
                String sender = user;
                storeMessageQueueToCSV(sender, receiver, message);
            }
            ...
            for (int i = 0; i < CSVReaderInJava.users.size(); i++) {
                String receiver = CSVReaderInJava.users.get(i).getUsername();
                String sender = user;
    ...
    // Method to store MessageQueue in CSV File
    private void storeMessageQueueToCSV(String sender, String receiver, String message) {
        // Get system timestamp
        String messageTimestamp = Utils.getDateAndTimeFromTimeStamp(System.currentTimeMillis(), Constants.DATE_FORMAT_FULL_DATE_TIME);
        
        // Create String to store in Message Queue using proper message structure
        String messageQueueNew = messageTimestamp + "  ->  " + sender + "  :  " + message + Constants.MESSAGE_QUEUE_SPLITTER_KEYWORD;
        ...
        // loop to iterate all usernames present in the CSV file
        for (int i = 0; i < CSVReaderInJava.users.size(); i++) {
            if (CSVReaderInJava.users.get(i).getUsername().equalsIgnoreCase(receiver)) {    // if username matches with existing username in the csv file then

                try {
                    // Store the details of each user in a CSV file
                    String messageQueue = CSVReaderInJava.users.get(i).getMessageQueue();
                    
                    // If the messageQueue contain blank/default message
                    if (messageQueue == null || messageQueue.isEmpty() || messageQueue.equals("") || messageQueue.equals(Constants.DEFAULT_MESSAGE_QUEUE)) {
                        // Initialize messageQueue with new message
                        messageQueue = messageQueueNew;
                    } else {
                        // else append messageQueue with new message
                        messageQueue = messageQueue + messageQueueNew;
                    }

                    // Rewrite messageQueue in CSV File
                    CSVReaderInJava.users.get(i).setMessageQueue(messageQueue);
                    rewriteCSV();
    ...
    // Method to broadcast updated User Log
    ...
    // Method to broadcast a message to close all connected clients
            ...
            try {
                while (true) {
                    ...
                    if (MESSAGE.equals(Constants.MESSAGE_TO_CLOSE_CLIENT)) {                // check if the message contains Specific Text to close and remove client
                        ...
                        break;
                    } else if (MESSAGE.equals(Constants.MESSAGE_TO_CHECK_MESSAGES)) {
                        System.out.println("checking message queue : name :" + name);
                        sendMessageQueueToClient(name);
                    } else {
    ...
    // Method to send message queue if client requests for it
    private void sendMessageQueueToClient(String user) {
        String messageQueue = "";
        String parts[];
        String messageStringFinal = "";
        ...
        for (int i = 0; i < CSVReaderInJava.users.size(); i++) {
            ...
            // if username matches with existing username in the csv file then
            if (CSVReaderInJava.users.get(i).getUsername().equalsIgnoreCase(user)) {    
                messageQueue = CSVReaderInJava.users.get(i).getMessageQueue();
                System.out.println("Pranit : StoreMessageTest : ServerScreen : sendMessageQueueToClient : messageQueue : " + messageQueue);
                break;
            }
        }

        // If messageQueue is not null then
        if (messageQueue != null) {
            // Split the message queue using particular keyword to differentiate between messages
            parts = messageQueue.split(Constants.MESSAGE_QUEUE_SPLITTER_KEYWORD);

            for (int i = 0; i < parts.length; i++) {
                // Create a final message String to send to client
                messageStringFinal = messageStringFinal + parts[i] + "\n";
            }
        }

        // Remove extra new line character from the final messageString
        messageStringFinal = messageStringFinal.substring(0, messageStringFinal.length() - 1);

        // Loop to check all clients
        for (HandleClient c : clients) {
            // Send message queue to the client who requested it
            if (c.getUserName().equalsIgnoreCase(user)) {
                c.sendMessage(messageStringFinal);
            }
        }

        // Call method to clear message queue once it is sent successfully to the intended client
        clearMessageQueueFromCSV(user);
    }

    // Method to clear message queue of given client
    private void clearMessageQueueFromCSV(String user) {
        for (int i = 0; i < CSVReaderInJava.users.size(); i++) {

            if (CSVReaderInJava.users.get(i).getUsername().equalsIgnoreCase(user)) {    // if username matches with existing username in the csv file then
                CSVReaderInJava.users.get(i).setMessageQueue(Constants.DEFAULT_MESSAGE_QUEUE);
                break;
            }
        }
    ...
    private void rewriteCSV() {
        ...
        for (int j = 0; j < CSVReaderInJava.users.size(); j++) {
            String messageQueue = CSVReaderInJava.users.get(j).getMessageQueue();

            System.out.println("Pranit : StoreMessageTest : ServerScreen : rewriteCSV : username : " + CSVReaderInJava.users.get(j).getUsername());
            System.out.println("Pranit : StoreMessageTest : ServerScreen : rewriteCSV : messageQueue : " + messageQueue);

            if (messageQueue == null || messageQueue.isEmpty() || messageQueue.equals("") || messageQueue.equals(Constants.DEFAULT_MESSAGE_QUEUE)) {
                users.add(new String[]{CSVReaderInJava.users.get(j).getUsername(), CSVReaderInJava.users.get(j).getTimestamp(), CSVReaderInJava.users.get(j).getStatus(), Constants.DEFAULT_MESSAGE_QUEUE});
            } else {
                users.add(new String[]{CSVReaderInJava.users.get(j).getUsername(), CSVReaderInJava.users.get(j).getTimestamp(), CSVReaderInJava.users.get(j).getStatus(), messageQueue});
